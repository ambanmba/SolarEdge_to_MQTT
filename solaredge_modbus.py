...
RETRIES = 3
TIMEOUT = 5  # Increased default timeout
UNIT = 1
...
INVERTER_STATUS_MAP = [
    "Undefined", "Off", "Sleeping", "Grid Monitoring", "Producing",
    "Producing (Throttled)", "Shutting Down", "Fault", "Standby"
]

BATTERY_STATUS_MAP = [
    "Off", "Standby", "Init", "Charge", "Discharge", "Fault", "Idle"
]
...
EXPORT_CONTROL_LIMIT_MAP = ["Total", "Per Phase"]

REACTIVE_POWER_CONFIG_MAP = [
    "Fixed CosPhi Mode", "Fixed Q Mode", "CosPhi(P) Mode",
    "Q(U) + Q(P) Mode", "RRCR Mode"
]
...
STOREDGE_AC_CHARGE_POLICY = {
    0: "Disabled", 1: "Always Allowed", 2: "Fixed Energy Limit",
    3: "Percent of Production"
}

STOREDGE_CHARGE_DISCHARGE_MODE = {
    0: "Off", 1: "Charge from excess PV power only", 2: "Charge from PV first",
    3: "Charge from PV and AC", 4: "Maximize export", 5: "Discharge to match load",
    7: "Maximize self consumption"
}

METER_REGISTER_OFFSETS = [0x0, 0xae, 0x15c]
BATTERY_REGISTER_OFFSETS = [0x0, 0x100]
...
class SolarEdge:
    ...
    parity = "N"
    baud = 115200
    wordorder = Endian.Big  # Updated to Endian.Big

    def __init__(self, host=False, port=False, timeout=TIMEOUT, retries=RETRIES, unit=UNIT, parent=False):
        ...
        if parent:
            ...
            self.unit = unit if unit else parent.unit
    ...
    def _read_holding_registers(self, address, length):
        """Read a block of holding registers with retries."""
        wordorder = Endian.Little if address in self.little_endian_registers else self.wordorder  # Updated to Endian.Little
        for _ in range(self.retries):
            if not self.connected():
                self.connect()
                time.sleep(0.1)
            result = self.client.read_holding_registers(address, length, slave=self.unit)
            if isinstance(result, ReadHoldingRegistersResponse) and len(result.registers) == length:
                return BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Big, wordorder=wordorder)
            time.sleep(0.1)
        return None

    def _decode_value(self, decoder, length, dtype, vtype):
        try:
            ...
            elif dtype in (registerDataType.UINT32, registerDataType.ACC32):
                decoded = decoder.decode_32bit_uint()
            elif dtype == registerDataType.UINT64:
                decoded = decoder.decode_64bit_uint()
            elif dtype in (registerDataType.FLOAT32, registerDataType.SEFLOAT):
                decoded = decoder.decode_32bit_float()
            elif dtype == registerDataType.STRING:
                decoded = decoder.decode_string(length * 2).decode("utf-8", errors="ignore").replace("\x00", "").rstrip()
            else:
                raise NotImplementedError(dtype)
            if decoded == SUNSPEC_NOTIMPLEMENTED[dtype.name] or decoded != decoded:  # NaN check
                return vtype(False)
            return vtype(decoded)
        except Exception:
            return vtype(False)

    def _read_all(self, registers):
        """Read all registers in a single bulk request per batch."""
        results = {}
        batches = {}
        
        # Group registers by batch
        for key, (addr, length, rtype, dtype, vtype, _, _, batch) in registers.items():
            if rtype != registerType.HOLDING:
                continue
            if batch not in batches:
                batches[batch] = []
            batches[batch].append((key, addr, length, dtype, vtype))

        # Process each batch
        for batch, reg_list in batches.items():
            if not reg_list:
                continue
            addr_min = min(addr for _, addr, _, _, _ in reg_list)
            addr_max = max(addr + length for _, addr, length, _, _ in reg_list)
            length = addr_max - addr_min

            decoder = self._read_holding_registers(addr_min, length)
            if not decoder:
                for key, _, _, _, vtype in reg_list:
                    results[key] = vtype(False)
                continue

            for key, addr, length, dtype, vtype in reg_list:
                offset = addr - addr_min
                if offset > 0:
                    decoder.skip_bytes(offset * 2)
                results[key] = self._decode_value(decoder, length, dtype, vtype)
    ...
    def read(self, key):
        """Read a single register (for compatibility)."""
        if key not in self.registers:
            raise KeyError(key)
        addr, length, rtype, dtype, vtype, _, _, _ = self.registers[key]
        if rtype != registerType.HOLDING:
            raise NotImplementedError(rtype)
        decoder = self._read_holding_registers(addr, length)
        return {key: self._decode_value(decoder, length, dtype, vtype) if decoder else vtype(False)}

    def read_all(self):
        """Read all registers in bulk."""
        return self._read_all(self.registers)

class Inverter(SolarEdge):
    def __init__(self, *args, **kwargs):
        self.model = "Inverter"
        self.wordorder = Endian.Big  # Updated to Endian.Big
        super().__init__(*args, **kwargs)

        self.little_endian_registers = {
            0xf700, 0xf701, 0xf702, 0xe004, 0xe005, 0xe006, 0xe008,
        ...
        self.registers = {
            ...
            "export_control_mode": (0xf700, 1, registerType.HOLDING, registerDataType.UINT16, int, "Export Control Mode", EXPORT_CONTROL_MODE_MAP, 5),
            ...
            "storage_control_mode": (0xe004, 1, registerType.HOLDING, registerDataType.UINT16, int, "Storage Control Mode", STOREDGE_CONTROL_MODE, 6),
            "storage_ac_charge_policy": (0xe005, 1, registerType.HOLDING, registerDataType.UINT16, int, "Storage AC Charge Policy", STOREDGE_AC_CHARGE_POLICY, 6),
            ...
            "storage_default_mode": (0xe00a, 1, registerType.HOLDING, registerDataType.UINT16, int, "Storage Charge/Discharge Default Mode", STOREDGE_CHARGE_DISCHARGE_MODE, 6),
            "rc_cmd_timeout": (0xe00b, 2, registerType.HOLDING, registerDataType.UINT32, int, "Remote Control Command Timeout", "s", 6),
            "rc_cmd_mode": (0xe00d, 1, registerType.HOLDING, registerDataType.UINT16, int, "Remote Control Command Mode", STOREDGE_CHARGE_DISCHARGE_MODE, 6),
    ...
    def meters(self):
        meters = [self._decode_value(self._read_holding_registers(addr, length), length, dtype, vtype) for addr, length, _, dtype, vtype, _, _, _ in self.meter_dids]
        return {f"Meter{idx + 1}": Meter(offset=idx, parent=self) for idx, v in enumerate(meters) if v and v != False}

    def batteries(self):
        batteries = [self._decode_value(self._read_holding_registers(addr, length), length, dtype, vtype) for addr, length, _, dtype, vtype, _, _, _ in self.battery_dids]
        return {f"Battery{idx + 1}": Battery(offset=idx, parent=self) for idx, v in enumerate(batteries) if v != 255 and v != False}
...
class Battery(SolarEdge):
    def __init__(self, offset=False, *args, **kwargs):
        ...
        self.wordorder = Endian.Little  # Updated to Endian.Little
        super().__init__(*args, **kwargs)
        ...
        self.registers = {
            ...
            "state_of_energy": (0xe184 + self.offset, 2, registerType.HOLDING, registerDataType.SEFLOAT, float, "State of Energy (SOE)", "%", 2),
